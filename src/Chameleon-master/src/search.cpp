...
template<bool Root>
Key Search::perft(Position &pos, Depth depth)
{
...
template Key Search::perft<true>(Position &, Depth);
...
void Thread::search()
{
  ...
  while (++rootDepth < DEPTH_MAX && !Signals.stop && (!Limits.depth || rootDepth <= Limits.depth))
  {
    ...
    if (mainThread)
      mainThread->bestMoveChanges /= 2, mainThread->failedLow = false;
...
namespace
{
  ...
  Value search(Position &pos, Stack *ss, Value alpha, Value beta, Depth depth, bool cutNode)
  {
    ...
    {
      Value rbeta = std::min(beta + 500, VALUE_INFINITE);
      Depth rdepth = depth - 9 * ONE_PLY;
        ...
        && (PvNode || ss->staticEval + 256 >= beta))
    {
      ...
      ss->skipEarlyPruning = true;
      value = -search < PvNode ? PV : NonPV > (pos, ss, alpha, beta, d, true);
      ss->skipEarlyPruning = false;
...
moves_loop: // When in check search starts from here
  ...
  /// Skill::pick_best() when playing with strength handicap, choose best move among a set of RootMoves
