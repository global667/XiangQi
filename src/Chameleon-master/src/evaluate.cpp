...
namespace
{
  ...
  {
    { 289, 344 }, { 221, 273 }, { 233, 201 }, { 46, 0 }, { 271, 0 }, { 307, 0 }, { 221, 273 }
  };
...
#define S(mg, eg) make_score(mg, eg)
  ...
  const Score MobilityBonus[][32] =
  {
    ...
    { S(-20, -20), S(-18, -18), S(-16, -16), S(-10, -10), S(-8, -8), S(-4, -4), S(0, 0), S(4, 2), S(8, 4), S(12, 6), S(16, 8), S(20, 10), S(24, 12), S(24, 12), S(24, 12), S(24, 12), S(24, 12), S(24, 12) }, // Rook
  };
  ...
  {
    { S(0, 0), S(7, 7), S(24, 49), S(24, 49), S(41, 100), S(41, 100), S(41, 100) },
    { S(0, 0), S(7, 7), S(15, 15), S(15, 15), S(35, 35), S(15, 35), S(24, 49) }
  };
...
#undef V
                        ...
                        const Bitboard *mobilityArea)
  {
    ...
    while ((s = *pl++) != SQ_NONE)
    {
      ...
      if (Pt == ROOK)
      {
        ...
        if (relative_rank(Us, s) >= RANK_5)
        {
          Bitboard alignedPawns = pos.pieces(Them, PAWN) & *RookAttacks[s];
          if (alignedPawns)
  ...
  template<>
  Score evaluate_pieces<false, WHITE, KING>(const Position &, EvalInfo &, Score *, const Bitboard *)
  {
    return SCORE_ZERO;
  }
  template<>
  Score evaluate_pieces< true, WHITE, KING>(const Position &, EvalInfo &, Score *, const Bitboard *)
  {
    return SCORE_ZERO;
  }
  ...
  Score evaluate_king(const Position &pos, const EvalInfo &ei)
  {
    ...
    if (ei.kingAttackersCount[Them])
    {
      ...
      attackUnits = std::min(72, ei.kingAttackersCount[Them] * ei.kingAttackersWeight[Them])
                    ...
                    - 64 * !pos.count<CANNON>(Them)
                    - mg_value(score) / 8;

      // Analyse the enemy's safe queen contact checks. Firstly, find the
      // undefended squares around the king reachable by the enemy queen...
      b = undefended & ei.attackedBy[Them][CANNON] & ~pos.pieces(Them);
      if (b)
      {
        // ...and then remove squares not supported by another enemy piece
        b &= ei.attackedBy[Them][PAWN] | ei.attackedBy[Them][KNIGHT]
             | ei.attackedBy[Them][BISHOP] | ei.attackedBy[Them][ADVISOR]
             | ei.attackedBy[Them][CANNON];
  ...
  Score evaluate_threats(const Position &pos, const EvalInfo &ei)
  {
    ...
    if (defended | weak)
    {
      ...
      b = (pos.pieces(Them, CANNON) | weak) & ei.attackedBy[Us][ROOK];
      while (b)
      ...
      if (b)
        score += ThreatByKing[b.more_than_one()];
    }
  ...
  Score evaluate_passed_pawns(const Position &pos, const EvalInfo &ei)
  {
    ...
    Bitboard b, defendedSquares, unsafeSquares;
    Score score = SCORE_ZERO;
    ...
    while (b)
    {
      ...
      if (rr)
      {
        ...
        {
          Bitboard bb = forward_bb(Them, s) & pos.pieces(ROOK, CANNON) & pos.attacks_from<ROOK>(s);
  ...
  Score evaluate_space(const Position &pos, const EvalInfo &ei)
  {
    ...
    int bonus = popcount<CNT_HW_POPCNT>((Us == WHITE ? safe << 32 : safe >> 32) | (behind & safe));
        ...
        && (sf == SCALE_FACTOR_NORMAL || sf == SCALE_FACTOR_ONEPAWN))
    {
      if (pos.opposite_bishops())
      {
        ...
        if (pos.non_pawn_material(WHITE) == BishopValueMg
          ...
          sf = pos.pieces(PAWN).more_than_one() ? ScaleFactor(31) : ScaleFactor(9);
