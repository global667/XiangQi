...
extern Key KnightLegMagic[SQUARE_NB];
extern Key KnightEyeMagic[SQUARE_NB];

extern Bitboard BishopEye[SQUARE_NB];
extern Bitboard BishopAttack[SQUARE_NB][16];
extern Key BishopEyeMagic[SQUARE_NB];
...
inline Bitboard flip_bb(const Bitboard& b)
{
  Bitboard t;

  for (Square sq = SQ_A0; sq < SQUARE_NB; sq++) {
    if (!(b & SquareBB[sq])) continue;
    t |= SquareBB[~sq];
  }
...
/// instance, in_front_bb(BLACK, RANK_4) will return the squares on ranks 1 and 2.
...
inline unsigned block_magic_index(Square s, const Bitboard &occ)
{
  ...
  Key *const Magics = Bt == KNIGHT_LEG ? KnightLegMagic : Bt == KNIGHT_EYE ? KnightEyeMagic : BishopEyeMagic;
...
inline Bitboard attacks_bb(Piece pc, Square s, const Bitboard &occupied)
{
  ...
  case CANNON:
    return cannon_attacks_bb(s, occupied);
  case KNIGHT:
    return knight_leg_attacks_bb(s, occupied);
  case BISHOP:
    return bishop_attacks_bb(s, occupied);
  case ADVISOR:
    return AdvisorAttack[s];
  case PAWN:
    return PawnAttackFrom[color_of(pc)][s];
  case KING:
    return KingAttack[s];
  default:
    return Bitboard();
